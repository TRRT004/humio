use crate::{DelayMs, InputDevice, Mouse, Point, ScrollAxis};
...
impl<D: InputDevice> HumanizedDevice<D> {
	...
	pub fn move_to_area(&mut self, area: &TargetArea, allow_error: bool) -> Result<(), String> {
		use rand::RngExt;
		let mut rng = rand::rng();

		let target = area.generate_click_point();
		let start = self.inner.location()?;
		log::trace!("Moving mouse: start={:?}, target={:?}", start, target);

		if allow_error && !self.should_bypass_failures() && rng.random_bool(self.config.overshoot_chance) {
			...
			log::trace!("Overshoot realization complete. Adjusting mouse to actual target {:?}", target);
	...
	pub fn click_area(&mut self, area: &TargetArea, button: Button, allow_error: bool) -> Result<(), String> {
		use rand::RngExt;
		let mut rng = rand::rng();
		...
		if let Some(failure) = triggered_failure {
			...
			match &failure {
				...
				ClickFailure::WrongButton(wrong_btn) => {
					log::trace!("Executing wrong button click: clicking {:?} instead of expected {:?}", wrong_btn, button);
			...
			self.execute_recovery_context(|d| d.run_default_click_recovery(&failure, area, button))?;
		} else {
	...
	pub fn click_area_flexible(
		...
		area: &TargetArea,
		button: Button,
		failures: &mut [(ClickFailure, f64, Box<dyn FnMut(&mut Self) -> Result<(), String> + '_>)],
	) -> Result<(), String> {
		use rand::RngExt;
		let mut rng = rand::rng();
	...
	fn run_default_click_recovery(&mut self, failure: &ClickFailure, area: &TargetArea, button: Button) -> Result<(), String> {
		use rand::RngExt;
		let mut rng = rand::rng();

		let reaction_delay = rng.random_range(250..=450);
		log::trace!("Built-in recovery: realizing click failure {:?} (waiting {}ms)...", failure, reaction_delay);
		std::thread::sleep(std::time::Duration::from_millis(reaction_delay));

		match failure {
			ClickFailure::Misclick | ClickFailure::MisclickTo(_) => {
				log::debug!("Built-in recovery: moving back to target area and retrying correct click.");
				self.move_to_area(area, false)?;
				sleep_gaussian_delay(DelayMs(80), 20);
				self.inner.click(button)?;
				sleep_gaussian_delay(DelayMs(80), 20);
			...
			ClickFailure::WrongButton(_) => {
				log::debug!("Built-in recovery: clicked wrong button. Tapping correct button {:?}.", button);
				sleep_gaussian_delay(DelayMs(80), 20);
				self.inner.click(button)?;
				sleep_gaussian_delay(DelayMs(80), 20);
			}
...
impl<D: InputDevice> Mouse for HumanizedDevice<D> {
	...
	fn scroll(&mut self, length: i32, axis: ScrollAxis) -> Result<(), String> {
		...
		let total_steps = length.abs() as f64;
