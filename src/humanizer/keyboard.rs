use crate::{InputDevice, Keyboard};
use super::failures::{FailureType, KeyboardFailure, KeyCombinationFailure};
...
impl<D: InputDevice> HumanizedDevice<D> {
	...
	pub fn text_humanized(&mut self, text: &str, allow_error: bool) -> Result<(), String> {
		use rand::RngExt;
		...
		while i < chars.len() {
			...
			let transposition_base = self.config.typing_failures.iter()
			...
			let transposition_prob = if is_failure_allowed {
				if let Some(ref calc) = self.chance_calculator {
					calc.calculate_chance(&FailureType::Keyboard(KeyboardFailure::Transposition), transposition_base)
				} else {
					transposition_base
				}
			} else {
			...
			if i + 1 < chars.len() && rng.random_bool(transposition_prob) {
				...
				log::debug!("Transposition error triggered! Swapping {:?} and {:?}", c, next_c);
				...
				log::trace!("Realized transposition error. Backspacing twice to delete {:?}", format!("{}{}", next_c, c));
			...
			let double_tap_base = self.config.typing_failures.iter()
				.find(|(f, _)| matches!(f, KeyboardFailure::DoubleTap))
				.map(|(_, p)| *p)
				.unwrap_or(0.0);
			let double_tap_prob = if is_failure_allowed {
				if let Some(ref calc) = self.chance_calculator {
					calc.calculate_chance(&FailureType::Keyboard(KeyboardFailure::DoubleTap), double_tap_base)
				} else {
					double_tap_base
				}
			} else {
			...
			let typo_base = self.config.typing_failures.iter()
			...
			if rng.random_bool(typo_prob) {
				...
				log::debug!("Typo generated: typed {:?} instead of expected {:?}", typo_char, c);
	...
	pub fn key_combination_humanized(&mut self, modifiers: &[Key], key: Key, allow_error: bool) -> Result<(), String> {
		use rand::RngExt;
		let mut rng = rand::rng();
		...
		if let Some(failure) = triggered_failure {
			...
			match failure {
				...
				KeyCombinationFailure::WrongKeyTap(wrong_key) => {
					log::trace!("Executing key combination error: tapped wrong target key {:?} instead of expected {:?}", wrong_key, key);
					for &mod_key in modifiers {
						self.inner.key(mod_key, Direction::Press)?;
						std::thread::sleep(std::time::Duration::from_millis(rng.random_range(15..45)));
					}
				...
				KeyCombinationFailure::ReleasedModifierEarly(early_mod) => {
					log::trace!("Executing key combination error: releasing modifier {:?} early", early_mod);
					for &mod_key in modifiers {
						self.inner.key(mod_key, Direction::Press)?;
						std::thread::sleep(std::time::Duration::from_millis(rng.random_range(15..45)));
					}
				...
				KeyCombinationFailure::ModifierStuck(stuck_mod) => {
					log::trace!("Executing key combination error: modifier {:?} got stuck (not released)", stuck_mod);
					for &mod_key in modifiers {
						self.inner.key(mod_key, Direction::Press)?;
						std::thread::sleep(std::time::Duration::from_millis(rng.random_range(15..45)));
					}
			...
			self.execute_recovery_context(|d| d.run_default_key_combo_recovery(&failure, modifiers, key))?;
		} else {
	...
	pub fn key_combination_flexible(
		...
		modifiers: &[Key],
		key: Key,
		failures: &mut [(KeyCombinationFailure, f64, Box<dyn FnMut(&mut Self) -> Result<(), String> + '_>)],
	) -> Result<(), String> {
		use rand::RngExt;
		let mut rng = rand::rng();
		...
		if let Some(idx) = triggered_index {
			...
			log::trace!("Simulated failure completed. Realizing error (waiting {}ms)...", reaction_delay);
			...
			// Normal combination
			log::trace!("Executing normal key combination: modifiers={:?}, key={:?}", modifiers, key);
			self.key_combination_normal(modifiers, key)?;
		}
	...
	fn run_default_key_combo_recovery(&mut self, failure: &KeyCombinationFailure, modifiers: &[Key], key: Key) -> Result<(), String> {
		use rand::RngExt;
		let mut rng = rand::rng();

		let reaction_delay = rng.random_range(200..=350);
		log::trace!("Built-in recovery: realizing key combo failure {:?} (waiting {}ms)...", failure, reaction_delay);
		std::thread::sleep(std::time::Duration::from_millis(reaction_delay));

		match failure {
			KeyCombinationFailure::MissedModifier(_) | KeyCombinationFailure::ReleasedModifierEarly(_) => {
			...
			KeyCombinationFailure::ModifierStuck(stuck_mod) => {
				log::debug!("Built-in recovery: modifier stuck. Releasing stuck modifier {:?} and retrying combination.", stuck_mod);
	...
	pub fn key_combination_normal(&mut self, modifiers: &[Key], key: Key) -> Result<(), String> {
		use rand::RngExt;
		let mut rng = rand::rng();
		...
		impl<'a, D: InputDevice> Drop for ModifierGuard<'a, D> {
			fn drop(&mut self) {
